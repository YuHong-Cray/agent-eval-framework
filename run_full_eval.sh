...
#    ./run_full_eval.sh --reset                 # 清理旧数据后重新测试
#    ./run_full_eval.sh --reset --adapter cray  # 一条命令：清理 → 评测 → 报告
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --help|-h)
            ...
            echo "  --reset            Clear old data before running"
...
if $SKIP_L1; then
    ...
    echo "🔬 Step 1: Layer 1 — 原子能力基准 (${L1_LABEL})"
    echo "──────────────────────────────────────────────────────────"
    L1_START=$(date +%s)
...
# ── Layer 2 ───────────────────────────────────────────────
if $SKIP_L2; then
    echo "⏭️  Step 2: Layer 2 — SKIPPED (needs LLM-as-Judge API key)"
else
    ...
    echo "🧪 Step 2: Layer 2 — 集成场景测试 (${L2_LABEL})"
    echo "──────────────────────────────────────────────────────────"
    L2_START=$(date +%s)
...
# ── Layer 3 ───────────────────────────────────────────────
if $SKIP_L3; then
    echo "⏭️  Step 3: Layer 3 — SKIPPED (needs LLM-as-Judge API key)"
else
    ...
    echo "🏗️  Step 3: Layer 3 — 长程项目测试 (${L3_LABEL})"
    echo "──────────────────────────────────────────────────────────"
    L3_START=$(date +%s)
